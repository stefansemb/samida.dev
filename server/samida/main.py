...
def with_runtime_info(system_message: ChatMessage, provider: ModelProvider, model: str) -> ChatMessage:
    # Merged into one system message rather than sent as a second one - some
    # chat templates (seen with a community Ollama GGUF) reject any system-role
    # message that isn't the very first one in the list with a Jinja error
    # ("System message must be at the beginning."), so two separate system
    # messages broke that model even though it worked for every built-in one.
    runtime_note = (
...
async def chat(
    ...
    try:
        ...
        turn = await provider.chat(
            [with_runtime_info(context.system_message, provider, resolved_model), *request.messages],
            resolved_model,
        )
...
async def conversation_chat(
    ...
    try:
        ...
        turn = await agent.run_turn(
            ...
            resolved_model,
            [
                with_runtime_info(context.system_message, provider, resolved_model),
                *model_history,
                current,
            ],
            workspace=workspace,
    ...
    *,
    ...
    try:
        ...
        turn = await agent.resume_after_decision(
            provider,
            record["model"],
            [with_runtime_info(context.system_message, provider, record["model"]), *model_history],
            call=call,
